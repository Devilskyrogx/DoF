-- DoF/UI/Nameplates.lua
-- HP на неймплейтах и всплывающий текст урона

local ADDON_NAME, DoF = ...

DoF.UI = DoF.UI or {}
DoF.UI.NameplateFrames = DoF.UI.NameplateFrames or {}
DoF.UI.FloatingTextPool = DoF.UI.FloatingTextPool or {}

function DoF.UI:UpdateNameplateFrame(np, unitId)
    ...
    if not self.NameplateFrames[np] then
        ...
        fr.icon:SetTexture("Interface\\AddOns\\DoF\\texture\\delves-scenario-heart-icon-2x")
        ...
        fr.text:SetFont(DoF.Config.FONT, 11, "OUTLINE")
    ...
    if data and data.hp ~= nil and data.maxHp then
        ...
        if dead then
            fr.text:SetText(DoF.L["ui.common.target_dead"])
...
function DoF.UI:UpdateAllNameplates()
...
-- Всплывающий текст
function DoF.UI:GetFloatingTextFrame(parent)
    ...
    ft.text:SetFont(DoF.Config.FONT, 18, "OUTLINE")
...
function DoF.UI:ShowFloatingText(unitName, text, r, g, b, isCrit)
    ...
    ft.text:SetFont(DoF.Config.FONT, isCrit and 24 or 18, "OUTLINE")
    ft:SetAlpha(1)
    ft:Show()
...
function DoF.UI:ShowAttackResult(unitName, resultType, damage)
    local texts = {
        crit_fail = { DoF.L["ui.nameplate.crit_fail"], 1, 0, 0, true },
        crit_success = { DoF.Locale:Format("ui.nameplate.crit_damage", damage or 0), 1, 0.84, 0, true },
        hit = { DoF.Locale:Format("ui.nameplate.hit", damage or 0), 0.2, 1, 0.2, false },
        miss = { DoF.L["ui.nameplate.miss"], 0.7, 0.7, 0.7, false },
        heal = { "+" .. (damage or 0) .. " HP", 0.2, 1, 0.2, false },
        crit_heal = { DoF.Locale:Format("ui.nameplate.crit_heal", damage or 0), 0.2, 1, 0.2, true },
    }
...
-- Алиасы
function DoF:UpdateAllNameplates() DoF.UI:UpdateAllNameplates() end
function DoF:ShowAttackResult(name, rtype, dmg) return DoF.UI:ShowAttackResult(name, rtype, dmg) end
